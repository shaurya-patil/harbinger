...
async function interpretRequirements(input) {
    const prompt = `You are an Interpreter Agent. Convert the following user instruction into clear, technical requirements.\n\nUser Input: ${input}\n\nTechnical Requirements:`;
    if (groqClient) {
...
async function executeTask(call, callback) {
    ...
    console.log(`[Interpreter Agent] Task ${task.id}: ${task.type}`);

    if (task.type === 'interpreter.interpret') {
        try {
            const result = await interpretRequirements(params.input);
            callback(null, {
                id: task.id,
                status: "success",
                result_uri: `text://${task.id}/requirements`,
                result_data: JSON.stringify({ requirements: result })
            });
        } catch (error) {
...
function healthCheck(call, callback) {
    callback(null, { status: "ok", capabilities: ["interpreter.interpret"] });
}
...
function main() {
    ...
    server.bindAsync(address, grpc.ServerCredentials.createInsecure(), () => {
        console.log(`[Interpreter Agent] Server running at ${address}`);
        server.start();
    });
}
main();
