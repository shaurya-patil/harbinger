...
async function generateTests(code) {
    const prompt = `You are a QA/Test Agent. Generate unit tests for the following code. Include edge cases.\n\nCode:\n${code}\n\nTests:`;
    if (groqClient) {
...
async function executeTask(call, callback) {
    ...
    console.log(`[QA Agent] Task ${task.id}: ${task.type}`);

    if (task.type === 'qa.generate_tests') {
        try {
            const result = await generateTests(params.code);
            callback(null, {
                id: task.id,
                status: "success",
                result_uri: `code://${task.id}/tests`,
                result_data: JSON.stringify({ tests: result })
            });
        } catch (error) {
...
function healthCheck(call, callback) {
    callback(null, { status: "ok", capabilities: ["qa.generate_tests"] });
}

function main() {
    const server = new grpc.Server();
    server.addService(taskProto.Agent.service, { ExecuteTask: executeTask, HealthCheck: healthCheck });
    const address = '127.0.0.1:50061';
    server.bindAsync(address, grpc.ServerCredentials.createInsecure(), () => {
        console.log(`[QA Agent] Server running at ${address}`);
        server.start();
    });
}
main();
